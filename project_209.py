...
try:
    ...
    def _on_collision(data):
        ...
        #Write a function for actor name
        actor_name=get_actor_display_name(data.other_actor)
        #print actor name here
        print("Collision happened with a", actor_name)
        #write a function for collision intensity
        collision_event_record = data.normal_impulse
        collision_intesity = math.sqrt(collision_event_record.x**2+collision_event_record.y**2+collision_event_record.z**2) 
        #print intensity
        print("Intensity of the collision is", collision_intesity)
